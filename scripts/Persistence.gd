...
func _ready():
	for action in InputMap.get_actions():
		if InputMap.action_get_events(action).size() != 0:
			config.set_value("Controles", action, InputMap.action_get_events(action)[0])
...
func load_control_settings():
	var keys = config.get_section_keys("Controles")
	
	for action in InputMap.get_actions():
		if keys.has(action):
			var value = config.get_value("Controles", action)
